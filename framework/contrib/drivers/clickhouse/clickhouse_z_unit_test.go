...
func TestDriverClickhouse_Insert(t *testing.T) {
	...
	var (
		insertUrl = "https://www.suxinwl.com"
		total     = 0
		item      = insertItem{
			Duration: 1,
			Url:      insertUrl,
...
func TestDriverClickhouse_Select(t *testing.T) {
	...
	gtest.AssertNil(err)
	temp, err := connect.Model("visits").Where("url", "www.suxinwl.com").Where("duration >= ", 1).One()
	gtest.AssertNil(err)
	...
	gtest.AssertNil(err)
	data, err := connect.Model("visits").Where("url", "www.suxinwl.com").Where("duration >= ", 1).All()
	gtest.AssertNil(err)
