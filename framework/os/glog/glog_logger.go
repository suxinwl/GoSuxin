...
import (
	...
	"github.com/suxinwl/GoSuxin/framework/internal/errors"
	...
	"github.com/suxinwl/GoSuxin/framework/os/gfpool"
	"github.com/suxinwl/GoSuxin/framework/os/gmlock"
...
func (l *Logger) print(ctx context.Context, level int, stack string, values ...any) {
	...
	if l.config.Flags&(F_FILE_LONG|F_FILE_SHORT|F_CALLER_FN) > 0 {
		callerFnName, path, line := gdebug.CallerWithFilter(
			[]string{consts.StackFilterKeyForSuxin},
			l.config.StSkip,
		)
...
func (l *Logger) GetStack(skip ...int) string {
	...
	if errors.IsStackModeBrief() {
		filters = append(filters, consts.StackFilterKeyForSuxin)
	}
