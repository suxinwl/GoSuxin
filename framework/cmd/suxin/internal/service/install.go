...
func (s serviceInstall) Run(ctx context.Context) (err error) {
	...
	if len(paths) <= 0 {
		mlog.Printf("no path detected, you can manually install suxin by copying the binary to path folder.")
		return
	}
	...
	if err != nil {
		mlog.Printf("install suxin binary to '%s' failed: %v", dstPath.dirPath, err)
		mlog.Printf("you can manually install suxin by copying the binary to folder: %s", dstPath.dirPath)
	} else {
