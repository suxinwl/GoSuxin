...
func ReplaceGeneratedContentGFV2(folderPath string) (err error) {
	return gfile.ReplaceDirFunc(func(path, content string) string {
		if gstr.Contains(content, `"github.com/gogf/gf`) && !gstr.Contains(content, `"github.com/suxinwl/GoSuxin/framework`) {
			content = gstr.Replace(content, `"github.com/gogf/gf"`, `"github.com/suxinwl/GoSuxin/framework"`)
			content = gstr.Replace(content, `"github.com/gogf/gf/`, `"github.com/suxinwl/GoSuxin/framework/`)
			content = gstr.Replace(content, `"github.com/suxinwl/GoSuxin/framework/contrib/`, `"github.com/gogf/gf/contrib/`)
			return content
		}
		return content
	}, folderPath, "*.go", true)
