...
// You can obtain one at https://github.com/suxinwl/templates/gf.
...
import (
	...
	"github.com/suxinwl/GoSuxin/framework/os/gproc"
	...
	"github.com/suxinwl/GoSuxin/framework/cmd/suxin/internal/cmd/geninit"
	...
	"github.com/suxinwl/GoSuxin/framework/cmd/suxin/internal/utility/utils"
)
...
const (
	cInitRepoPrefix  = `github.com/suxinwl/templates/`
	cInitMonoRepo    = `template-mono`
	cInitMonoRepoApp = `template-mono-app`
	cInitSingleRepo  = `template-single`
	cInitBrief       = `create and initialize an empty Suxin project`
	cInitEg          = `
suxin init my-project
suxin init my-project -g "example.com/myproject"
suxin init my-project --sdk /path/to/GoSuxin/framework
suxin init my-mono-repo -m
suxin init my-mono-app -a
`
	cInitNameBrief = `
...
var defaultTemplates = []struct {
	...
	{"template-single", "github.com/suxinwl/templates/template-single", "Single project template"},
	{"template-mono", "github.com/suxinwl/templates/template-mono", "Mono-repo project template"},
}
...
type cInitInput struct {
	SDK         string `name:"sdk" brief:"local Suxin framework source directory (optional)"`
	g.Meta      `name:"init"`
	...
	Update      bool   `name:"update" short:"u" brief:"online updates are unavailable in Suxin V1.0.0" orphan:"true"`
...
func (c cInit) Index(ctx context.Context, in cInitInput) (out *cInitOutput, err error) {
	if in.Repo != "" || in.Update || in.SelectVer || in.Interactive {
		return nil, fmt.Errorf("Suxin V1.0.0 uses built-in templates; online template/update services are not enabled")
	}
	if strings.TrimSpace(in.Name) == "" {
		return nil, fmt.Errorf("usage: suxin init PROJECT [--sdk FRAMEWORK_DIRECTORY]")
	}
	if !gfile.IsEmpty(in.Name) {
		return nil, fmt.Errorf("target directory must be empty")
	}
	if in.SDK != "" && !gfile.Exists(gfile.Join(in.SDK, "go.mod")) {
		return nil, fmt.Errorf("SDK go.mod not found")
	}
	out, err = c.initFromBuiltin(ctx, in)
	if err != nil {
		return out, err
	}
	if in.SDK != "" && gfile.Exists(gfile.Join(in.Name, "go.mod")) {
		sdk := gfile.RealPath(in.SDK)
		err = gfile.PutContentsAppend(gfile.Join(in.Name, "go.mod"), fmt.Sprintf("\nreplace github.com/suxinwl/GoSuxin/framework => %q\n", sdk))
	}
	return out, err
}
...
func (c cInit) initFromRemote(ctx context.Context, in cInitInput) (out *cInitOutput, err error) {
	...
	if name != "" && name != "." {
		mlog.Printf(`you can now run "cd %s && suxin run main.go" to start your journey, enjoy!`, name)
	}
	return
}
...
func (c cInit) initFromBuiltin(ctx context.Context, in cInitInput) (out *cInitOutput, err error) {
	...
	// Update the Suxin version.
	if in.Update {
		mlog.Print("update Suxin...")
		// go get -u github.com/suxinwl/GoSuxin/framework@latest
		updateCommand := `go get -u github.com/suxinwl/GoSuxin/framework@latest`
		if in.Name != "." {
	...
	mlog.Print("initialization done! ")
	if !in.Mono {
		enjoyCommand := `suxin run main.go`
		if in.Name != "." {
...
func (c cInit) initInteractive(ctx context.Context, in cInitInput) (out *cInitOutput, err error) {
	...
	// Ask about update
	fmt.Print("Update to latest Suxin version? [y/N]: ")
	input, err = reader.ReadString('\n')
	if err != nil {
		mlog.Fatalf("failed to read input: %v", err)
		return
