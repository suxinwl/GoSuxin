...
const (
	cRunUsage = `suxin run FILE [OPTION]`
	cRunBrief = `running go codes with hot-compiled-like feature`
	cRunEg    = `
suxin run main.go
suxin run main.go --args "server -p 8080"
suxin run main.go -mod=vendor
suxin run main.go -w internal,api
suxin run main.go -i ".git,node_modules"
`
	cRunDc = `
...
type (
	cRunInput struct {
		g.Meta         `name:"run" config:"suxincli.run"`
