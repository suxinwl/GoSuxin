...
var Build = cBuild{
	nodeNameInConfigFile: "suxincli.build",
...
const (
	...
	cBuildEg          = `
suxin build main.go
suxin build main.go --ps public,template
suxin build main.go --cgo
suxin build main.go -m none 
suxin build main.go -n my-app -a all -s all
suxin build main.go -n my-app -a amd64,386 -s linux -p .
suxin build main.go -n my-app -v 1.0 -a amd64,386 -s linux,windows,darwin -p ./docker/bin
`
	cBuildDc = `
...
type cBuildInput struct {
	g.Meta        `name:"build" config:"suxincli.build"`
...
func (c cBuild) Index(ctx context.Context, in cBuildInput) (out *cBuildOutput, err error) {
	...
	ldFlags := fmt.Sprintf(
		`-X 'github.com/suxinwl/GoSuxin/framework/os/gbuild.builtInVarStr=%v'`,
