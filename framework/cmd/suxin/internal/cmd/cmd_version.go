...
// You can obtain one at https://github.com/suxinwl/GoSuxin/framework.
...
import (
	...
	"github.com/suxinwl/GoSuxin/framework"
...
func (c cVersion) Index(ctx context.Context, in cVersionInput) (*cVersionOutput, error) {
	...
	detailBuffer.appendLine(0, "Welcome to Suxin!")
	...
	if ok {
		...
		detailBuffer.appendLine(1, fmt.Sprintf("Suxin Version(go.mod): %s", getSuxinVersion(2)))
	} else {
		v, err := c.getGFVersionOfCurrentProject()
		if err == nil {
			detailBuffer.appendLine(1, fmt.Sprintf("Suxin Version(go.mod): %s", v))
		} else {
			detailBuffer.appendLine(1, fmt.Sprintf("Suxin Version(go.mod): %s", err.Error()))
		}
	}
	...
	if info.Suxin == "" {
		...
		detailBuffer.appendLine(1, fmt.Sprintf("Built Suxin Version: %s", gf.VERSION))
	} else {
		if info.Git == "" {
			info.Git = "none"
		}
		detailBuffer.appendLine(1, fmt.Sprintf("Built Go Version: %s", info.Golang))
		detailBuffer.appendLine(1, fmt.Sprintf("Built Suxin Version: %s", info.Suxin))
	...
	detailBuffer.appendLine(1, "Docs: https://www.suxinwl.com")
...
// getSuxinVersion returns the Suxin version of current project using.
func getSuxinVersion(indentLevel int) (gfVersion string) {
	pkgInfo, err := gproc.ShellExec(context.Background(), `go list -f "{{.Path}}@{{.Version}}" -m all`)
	if err != nil {
		return "cannot find go.mod"
	}
	pkgList := gstr.Split(pkgInfo, "\n")
	for _, v := range pkgList {
		if strings.HasPrefix(v, "github.com/suxinwl/GoSuxin/framework") {
...
// getGFVersionOfCurrentProject checks and returns the Suxin version current project using.
func (c cVersion) getGFVersionOfCurrentProject() (string, error) {
	...
	if gfile.Exists(goModPath) {
		...
		for _, line := range lines {
			...
			line = gstr.Trim(line)
			// Version 1.
			match, err := gregex.MatchString(`^github\.com/suxinwl/GoSuxin/framework\s+(.+)$`, line)
			if err != nil {
				return "", err
			}
			if len(match) <= 1 {
				// Version > 1.
				match, err = gregex.MatchString(`^github\.com/suxinwl/GoSuxin/framework/v\d\s+(.+)$`, line)
				if err != nil {
					return "", err
				}
			}
		...
		return "", gerror.New("cannot find Suxin requirement in go.mod")
	} else {
