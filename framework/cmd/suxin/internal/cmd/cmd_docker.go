...
const (
	...
	cDockerBrief = `build docker image for current Suxin project`
	cDockerEg    = `
gf docker 
...
It runs "suxin build" firstly to compile the project to binary file.
...
`
	cDockerMainBrief        = `main file path for "suxin build", it's "main.go" in default. empty string for no binary build`
...
type cDockerInput struct {
	g.Meta      `name:"docker" config:"suxincli.docker"`
...
func (c cDocker) Index(ctx context.Context, in cDockerInput) (out *cDockerOutput, err error) {
	...
	if in.Main != "" && in.Build != "" {
		...
		if in.Main != "" {
			if err = gproc.ShellRun(ctx, fmt.Sprintf(`suxin build %s %s`, in.Main, in.Build)); err != nil {
