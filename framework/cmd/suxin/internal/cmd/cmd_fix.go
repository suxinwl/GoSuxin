...
type cFix struct {
	g.Meta `name:"fix" brief:"auto fixing codes after upgrading to new Suxin version" usage:"gf fix" `
}
...
func (c cFix) Index(ctx context.Context, in cFixInput) (out *cFixOutput, err error) {
	...
	if in.Version == "" {
		...
		if in.Version == "" {
			mlog.Print(`no Suxin usage found, exit fixing`)
			return
		}
		mlog.Debugf(`current Suxin version auto detect "%s"`, in.Version)
	}
...
func (c cFix) doFix(in cFixInput) (err error) {
	...
	for _, item := range items {
		if gstr.CompareVersionGo(in.Version, item.Version) < 0 {
			mlog.Debugf(
				`current Suxin or contrib package version "%s" is lesser than "%s", nothing to do`,
...
// doFixV23 fixes code when upgrading to Suxin v2.3.
...
// doFixV25 fixes code when upgrading to Suxin v2.5.
