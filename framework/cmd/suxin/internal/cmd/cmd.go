...
import (
	...
	"github.com/suxinwl/GoSuxin/framework/cmd/suxin/internal/service"
...
type cGF struct {
	g.Meta `name:"suxin" ad:"{cGFAd}"`
}

const (
	cGFAd = `
ADDITIONAL
    Use "suxin COMMAND -h" for details about a command.
`
)
...
func (c cGF) Index(ctx context.Context, in cGFInput) (out *cGFOutput, err error) {
	...
	if data, isInstalled := service.Install.IsInstalled(); !isInstalled {
		mlog.Print("hi, it seems it's the first time you installing suxin cli.")
		answer = gcmd.Scanf("do you want to install suxin(%s) binary to your system? [y/n]: ", gf.VERSION)
	} else if !data.IsSelf {
		mlog.Print("hi, you have installed suxin cli.")
		answer = gcmd.Scanf("do you want to install suxin(%s) binary to your system? [y/n]: ", gf.VERSION)
	}
