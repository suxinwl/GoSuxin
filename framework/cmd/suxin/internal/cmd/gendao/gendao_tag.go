...
const (
	CGenDaoConfig = `suxincli.gen.dao`
	CGenDaoUsage  = `suxin gen dao [OPTION]`
	CGenDaoBrief  = `automatically generate go files for dao/do/entity`
	CGenDaoEg     = `
suxin gen dao
suxin gen dao -l "mysql:root:12345678@tcp(127.0.0.1:3306)/test"
suxin gen dao -p ./model -g user-center -t user,user_detail,user_login
suxin gen dao -r user_
`
...
CONFIGURATION SUPPORT
    ...
    The configuration node name is "suxincli.gen.dao", which also supports multiple databases, for example(config.yaml):
	suxincli:
	  gen:
		dao:
...
`
	...
	CGenDaoBriefLink              = `database configuration, the same as the ORM configuration of Suxin`
