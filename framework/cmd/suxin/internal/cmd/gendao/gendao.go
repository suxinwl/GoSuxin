...
import (
	...
	"github.com/suxinwl/GoSuxin/framework/os/gview"
...
func getImportPartContent(ctx context.Context, source string, isDo bool, appendImports []string) string {
	...
	if isDo {
		packageImportsArray.Append(`"github.com/suxinwl/GoSuxin/framework/frame/g"`)
	}

	// Time package recognition.
	if strings.Contains(source, "gtime.Time") {
		packageImportsArray.Append(`"github.com/suxinwl/GoSuxin/framework/os/gtime"`)
	...
	if strings.Contains(source, "gjson.Json") {
		packageImportsArray.Append(`"github.com/suxinwl/GoSuxin/framework/encoding/gjson"`)
	}
