...
func (m *TencentCOS) InitClient(config Config) {
	m.Ready = false
	m.Config = config
	u, _ := url.Parse(fmt.Sprintf("https://%v.cos.%v.myqcloud.com", m.Config.BucketName, m.Config.Region))
...
// 上传文件
func (m *TencentCOS) UploadFile(file *ghttp.UploadFile) (url, cover_url string, err error) {
	if !m.Ready {
	...
	defer fd.Close()
	//注意:文件名称必须以tencentcos开头
	path_name := fmt.Sprintf("%v/%v/%v", m.Config.DirPath, time.Now().Format("20060102"), MakeFileName(file, "tencentcos"))
	url = path_name
	_, err = m.Client.Object.Put(context.Background(), path_name, fd, nil)
	return
}
...
// 删除文件
func (m *TencentCOS) RemoveFile(fileUrl string) error {
	if !m.Ready {
		return errors.New("not ready")
	}
